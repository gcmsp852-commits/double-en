...
// 笘・繧ｷ繧ｹ繝・Β證怜捷蛹門ｾｩ蜿ｷ逕ｨ・壼梛蠑乗ュ蝣ｱ縺ｮ繝槭せ繧ｯ逡ｪ蜿ｷ繧偵ン繝・ヨ蜿崎ｻ｢縺励※豁｣縺励＞formatInfo繧貞ｾ励ｋ
function buildFormatBits(data) {
    var G15 = 0x537;
    var G15_MASK = 0x5412;
    var d = data << 10;
    var g15digit = 0;
...
var SYS_ENC_FORMAT_INFO_TABLE = FORMAT_INFO_TABLE.map(function (entry) {
    var info = entry.formatInfo;
    var encryptedMask = info.dataMask ^ 0x7;
    var encryptedData = (info.errorCorrectionLevel << 3) | encryptedMask;
    return {
        bits: buildFormatBits(encryptedData),
        formatInfo: {
            errorCorrectionLevel: info.errorCorrectionLevel,
            dataMask: info.dataMask
        }
    };
});
function decodeFormatInfoWithTable(topLeftBits, topRightBottomRightBits, table) {
    var bestDifference = Infinity;
    var bestFormatInfo = null;
    for (var _i = 0, table_1 = table; _i < table_1.length; _i++) {
        var _a = table_1[_i], bits = _a.bits, formatInfo = _a.formatInfo;
        if (bits === topLeftBits || bits === topRightBottomRightBits) {
            return formatInfo;
        }
        var difference = numBitsDiffering(topLeftBits, bits);
        if (difference < bestDifference) {
            bestFormatInfo = formatInfo;
            bestDifference = difference;
        }
        if (topLeftBits !== topRightBottomRightBits) {
            difference = numBitsDiffering(topRightBottomRightBits, bits);
            if (difference < bestDifference) {
                bestFormatInfo = formatInfo;
                bestDifference = difference;
            }
...
function readFormatInformationSysEnc(matrix) {
    ...
    return decodeFormatInfoWithTable(topLeftFormatInfoBits, topRightBottomRightFormatInfoBits, SYS_ENC_FORMAT_INFO_TABLE);
}
...
function findAlignmentPattern(matrix, alignmentPatternQuads, topRight, topLeft, bottomLeft) {
    ...
    var alignmentPatterns = alignmentPatternQuads
        ...
        // 笘・繝・う繝ｳQR蟇ｾ遲厄ｼ壻ｺ域Φ菴咲ｽｮ縺九ｉ縺ｮ霍晞屬縺瑚ｿ代＞繧ゅ・縺ｮ縺ｿ謗｡逕ｨ縲・        // 繝・う繝ｳ縺ｧ縺ｯ蛻･QR縺ｮ繝弱う繧ｺ縺碁撼蟶ｸ縺ｫ螟壹＞縺後〃er5遲峨〒蠕ｮ螯吶↓縺壹ｌ繧句ｴ蜷医ｒ閠・・縺励※3繝｢繧ｸ繝･繝ｼ繝ｫ縺ｾ縺ｧ險ｱ螳ｹ縲・        if (d > moduleSize * 3) {
            return;
        }
        ...
        .sort(function (a, b) { return a.score - b.score; });
    // 笘・modulesBetweenFinderPatterns >= 25 縺ｫ螟画峩・・5 竕・30繝｢繧ｸ繝･繝ｼ繝ｫ縺ｧ繧・expectedAlignmentPattern 縺ｫ繝輔か繝ｼ繝ｫ繝舌ャ繧ｯ蜿ｯ閭ｽ縺ｫ・・    var alignmentPattern = modulesBetweenFinderPatterns >= 25 && alignmentPatterns.length ? alignmentPatterns[0] : expectedAlignmentPattern;
